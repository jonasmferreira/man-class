			...
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
				<tbody>
					...
					<tr>
						<td>Nivel:</td>
						<td>
							<select name="usuario_nivel_id" id="usuario_nivel_id" class="obrigatorio">
								<?	foreach($aNivel AS $k=> $v):?>
								<option value="<?=$v['usuario_nivel_id']?>" <?=$usuario['usuario_nivel_id'] == $v['usuario_nivel_id'] ? "selected='selected'" : '' ?>><?=$v['usuario_nivel_titulo']?></option>
								<?	endforeach;?>
							</select>
					...
					<tr>
						<td>Status:</td>
						<td>
							<select name="usuario_status_id" id="usuario_status_id" class="obrigatorio">
								<?	foreach($aStatus AS $v):?>
								<option value="<?=$v['usuario_status_id']?>" <?=$usuario['usuario_status_id'] == $v['usuario_status_id'] ? "selected='selected'" : '' ?>><?=$v['usuario_status_titulo']?></option>
								<?	endforeach;?>
							</select>
						</td>
					</tr>
